...
* [`src/feuerwehr_strichliste/`](src/feuerwehr_strichliste/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/feuerwehr_strichliste/core.cljs): contains the SPA entry point, `init`
* [`test/feuerwehr_strichliste/`](test/feuerwehr_strichliste/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `feuerwehr-strichliste` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/feuerwehr_strichliste/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns feuerwehr-strichliste.example
  (:require [feuerwehr-strichliste.config :as config])
